---
name: gcp-project-discoverer-wf
description: Discovers existing GCP resources in a given project using gcloud CLI and returns a structured as-is inventory. Invoked by terraform-creator-wf when bootstrapping a new terraform folder to cross-reference live infrastructure. Also invoke directly when you need an inventory of a GCP project.
metadata:
  skill-type: workflow
  version: "1.0"
...
Queries a live GCP project via `gcloud` CLI to produce a structured inventory of existing resources. The inventory is consumed by `terraform-creator-wf` to inform HCL generation — ensuring proposed code reflects what is already deployed and avoids duplicating resources.

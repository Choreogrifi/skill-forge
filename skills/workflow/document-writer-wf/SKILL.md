...
(e.g. "a Node.js REST API for managing orders", "the checkout service architect-smeure")
```
...
Select a document template:
  ...
  2. Mermaid diagram      — architect-smeure, flow, sequence, or state diagram
  3. Other                — describe the document type you need

Enter 1–3:
```

- `1` → activate `write-readme-wf`, pass the subject.
- `2` → activate `mermaid-drawer-wf`, pass the subject.
- `3` → ask: "Describe the document type (e.g. runbook, API reference, onboarding guide, ADR)." Then activate `write-readme-wf` with a generic document mode, or inform the user that this template is not yet available and suggest the closest match. Log the requested type for future template addition.

### 3b. Existing Document Operations

If the user selected 2 → activate `review-document-wf`.
If the user selected 3 → activate `update-document-wf`.
If the user selected 4 → activate `add-document-wf`.
...
- **mermaid-drawer-wf is external** — it is an existing skill; do not replicate its logic here.

## Subflows

| File | Load when |
|---|---|
| `subflows/document-writer-adr-sf.md` | User wants to create an Architecture Decision Record |

## Related Skills

- `write-readme-wf` — drafts a discoverable Git README.md from the 2026 template
- `mermaid-drawer-wf` — generates Mermaid diagram code (existing skill)
- `review-document-wf` — reviews any document against quality and structure standards
- `update-document-wf` — modifies specific sections of an existing document with confirmation
- `add-document-wf` — appends new sections or content to an existing document with confirmation

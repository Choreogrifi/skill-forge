---
name: add-document-wf
description: Appends new sections, content blocks, or entries to an existing document. Shows the proposed addition in context before writing. Activated by document-writer-wf or invoke directly to extend any document.
metadata:
  skill-type: workflow
  version: "1.0"
...
- **Append-only by default** — do not modify existing content unless the user explicitly requests it. If modification is needed, delegate to `update-document-wf`.

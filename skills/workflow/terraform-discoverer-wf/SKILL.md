---
name: terraform-discoverer-wf
description: Analyses a codebase to infer the cloud infrastructure it requires and proposes a structured Terraform resource plan. Invoked by terraform-creator-wf when no ./terraform folder exists. Also invoke directly when you want a Terraform proposal derived from application code alone.
metadata:
  skill-type: workflow
  version: "1.0"
...
Reads the application codebase in the current working directory and infers what GCP infrastructure the application requires. Produces a structured resource proposal that `terraform-creator-wf` consumes to pre-populate its HCL generation step. Does not write any files — output only.
...
Return a structured proposal — this is consumed directly by `terraform-creator-wf`:
...
- **No credentials** — if secret values are visible in code, flag them as a security-sme issue; do not include them in the proposal.

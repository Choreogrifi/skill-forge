...
<dict>
  ...
  <array>
    ...
    <string>run</string>
    <string>~/.llm-assets/mcp/skills_server.py</string>
  </array>
...
|---|---|---|---|
| Filesystem | `@modelcontextprotocol/server-filesystem` | stdio | manage-skills-wf: safer `references/` writes |
| Git | `mcp-server-git` (uvx) | stdio | git-commit-wf: richer diff/log data |
| Bandit wrapper | custom Python | stdio | security-sme: static analysis on staged files |
| Skills validator | custom Python | stdio | manage-skills-wf: SKILL.md frontmatter check |

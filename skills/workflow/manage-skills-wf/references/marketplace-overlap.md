...
|---|---|---|---|
| `git-commit-wf` | `commit-commands` | Full overlap — both automate git commit/push/PR workflows | **Do not install** |
| `security-sme` | `security-sme-guidance` | Full overlap — both enforce OWASP and GCP IAM security-sme rules | **Do not install** |
| `engineer-sme` | `feature-dev` | Partial overlap — feature-dev adds multi-agent orchestration | Avoid — custom skill preferred |
| `engineer-sme` | `code-simplifier` | Partial overlap — code-simplifier focuses on refactoring only | Avoid — custom skill preferred |
| `architect-sme` | `code-review` | Partial overlap — code-review automates PR review agents | Avoid — custom skill preferred |
| `tester-sme` | `pr-review-toolkit` | Partial overlap — pr-review-toolkit automates PR-level test checks | Avoid — custom skill preferred |
...
| `skill-creator` | Skill scaffolding and evaluation tooling | Complements `manage-skills-wf` |

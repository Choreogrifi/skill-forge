---
name: terraform-creator-wf
description: Guided workflow to scaffold Terraform HCL in the current project. Detects an existing ./terraform folder and reads environment state to understand as-is; if absent, bootstraps the folder and invokes terraform-discoverer-wf (codebase analysis) and optionally gcp-project-discoverer-wf (live GCP inventory). Invoke when adding a new module, resource, or workload to a cloud project.
metadata:
  skill-type: workflow
  version: "1.0"
...
Scaffolds new Terraform HCL that conforms to the project's established conventions. Reads existing state to understand what is already deployed before generating anything new. If no Terraform folder exists, bootstraps one using `terraform-discoverer-wf` to analyse the codebase and optionally `gcp-project-discoverer-wf` to cross-reference live GCP infrastructure.
...
- Invoke `terraform-discoverer-wf` to analyse the codebase and produce a proposed resource plan. Hold this output as the **discovery baseline**.
- Ask the user: "Would you like me to cross-reference a live GCP project to refine the proposal? (yes / no)"
  - If yes: invoke `gcp-project-discoverer-wf`, which will prompt for the GCP project ID. Merge its inventory with the discovery baseline — resources already present in GCP are marked `[existing]`; new resources are marked `[new]`.
...
- **SME skills are required for bootstrap:** `terraform-discoverer-wf` must always be invoked when no `./terraform` folder exists — never skip it. `gcp-project-discoverer-wf` is optional but should always be offered.
...
- `terraform-discoverer-wf` — invoked during Step 1 bootstrap to analyse the codebase and propose required GCP resources
- `gcp-project-discoverer-wf` — invoked during Step 1 bootstrap (optional) to cross-reference live GCP infrastructure against the discovery proposal
